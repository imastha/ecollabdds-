...
public class Admin_ClientApplication_Pages {
	...
	public void CA_AddForm_NoData_Tests(SoftAssert Assert) throws InterruptedException {
		...
		Assert.assertEquals(SelectOrg_label.getText(), "Select Organization*", "SelectOrg label is incorrect. ,");
		...
		Assert.assertEquals(clientAdmin_username_hint.getText(),
				"[Used to login and configure client application parameters. Username should be alphanumeric [allowed special characters dot(.), @, underscore(_), dash(-)] and length 3 to 50]",
				"clientAdmin_username_hint is incorrect. ,");
	...
	public void List_CA_Action_Update_CApp(SoftAssert Assert) throws IOException {
		...
		Assert.assertEquals(SelectOrg_label.getText(), "Select Organization*", "Select Organization label missing. ,");
		...
		Assert.assertEquals(statusOfCA_label.getText(), "Status of Client Application: *",
				"status Of Client Application label missing. ,");
		if (statusOfCA_checkbox.isDisplayed()) {
			if (statusOfCA_checkbox.isSelected() == false) {
				statusOfCA_checkbox.click();
			}
		}
		Assert.assertEquals(UserCode_label.getText(), "User Code:", "User Code label missing. ,");
		...
		Assert.assertEquals(statusOfClientAdmin_label.getText(), "Status of Client Admin: *",
				"status Of Client Admin label missing. ,");
		...
		if (SearchButton.isEnabled()) {
			...
			if (clientSearchButton.isEnabled()) {
				...
				if (clientSearchButton.isEnabled()) {
					...
					Assert.assertTrue(clientSaveButton.isDisplayed(),
							"client save button not displayed in search>action>update");
					Assert.assertTrue(clientSaveButton.isEnabled(),
							"client save button not displayed in search>action>update");
